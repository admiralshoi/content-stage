...
class MediaStream {
    ...
    public function setTmpUpload($FILES): ?string {
        if(empty($FILES)) return null;
        $tmpFileDir = "objects/tmp/" . $this->requestingUsersId . "/";
        if(!is_dir(ROOT . $tmpFileDir)) mkdir(ROOT . $tmpFileDir);

        $object = $FILES[array_keys($FILES)[0]];

        foreach (array("tmp_name", "name", "size", "type") as $key) if(!array_key_exists($key, $object)) return null;

        $originalFn = $object["name"];
        $tmpFile = $object["tmp_name"];
        $destination = $tmpFileDir . $originalFn;
    ...
    public function mediaTypeDetails(string $type): array {
        $details = array(
            ...
            "image" => array(
                ...
                "valid_types" => array("image/jpeg"=>"jpeg","image/jpg"=>"jpg","image/png"=>"png","image/gif"=>"gif"),
