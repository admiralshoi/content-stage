...
class CreatorRelations {
    ...
    public function getByX(array $params = array(), array $fields = array(), string $customSql = ""): array {
        ...
        return $this->crud->retrieve("creator_relations",$params, $fields,$customSql);
    }
    public  function create(array $params): bool {
        ...
        return $this->crud->create("creator_relations", array_keys($params), $params);
    }
    public function update(array $params, array $identifier): bool {
        if(!$this->access(MODIFY_ACTION)) return false;
        if(!array_key_exists("updated_at", $params)) $params["updated_at"] = time();
        return $this->crud->update("creator_relations", array_keys($params), $params, $identifier);
    }
    public  function delete(array $identifier): bool {
        if(!$this->access(MODIFY_ACTION)) return false;
        return $this->crud->delete("creator_relations", $identifier);
    }
    ...
    public function exists(string|int $lookupId): bool {
        return !empty($this->getByX(array("lookup_id" => $lookupId, "user_id" => $this->requestingUsersId)));
    }



    public function getByUserId(string|int $id = 0, array $fields = array()): array {
        if(empty($id)) $id = $this->requestingUsersId;
        return $this->getByX(array("user_id" => $id), $fields);
    }
